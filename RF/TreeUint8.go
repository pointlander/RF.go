//a random forest implemtation in GoLang
package RF

import "math/rand"

type TreeNodeUint8 struct {
	ColumnNo int //column number
	Value    uint8
	Left     *TreeNodeUint8
	Right    *TreeNodeUint8
	Labels   map[string]int
}

type TreeUint8 struct {
	Root *TreeNodeUint8
}

func getSamplesUint8(ary [][]uint8, index []int) [][]uint8 {
	result := make([][]uint8, len(index))
	for i := 0; i < len(index); i++ {
		result[i] = ary[index[i]]
	}
	return result
}

func getBestGainUint8(samples [][]uint8, c int, samples_labels []string, current_entropy float64) (float64, uint8, int, int) {
	var best_value uint8
	best_gain := 0.0
	best_total_r := 0
	best_total_l := 0

	uniq_values := make(map[uint8]int)
	for i := 0; i < len(samples); i++ {
		uniq_values[samples[i][c]] = 1
	}

	for value, _ := range uniq_values {
		map_l := make(map[string]float64)
		map_r := make(map[string]float64)
		total_l := 0
		total_r := 0
		for j := 0; j < len(samples); j++ {
			if samples[j][c] <= value {
				total_l += 1
				map_l[samples_labels[j]] += 1.0
			} else {
				total_r += 1
				map_r[samples_labels[j]] += 1.0
			}
		}

		p1 := float64(total_r) / float64(len(samples))
		p2 := float64(total_l) / float64(len(samples))

		new_entropy := p1*getEntropy(map_r, total_r) + p2*getEntropy(map_l, total_l)
		//fmt.Println(new_entropy,current_entropy)
		entropy_gain := current_entropy - new_entropy

		if entropy_gain >= best_gain {
			best_gain = entropy_gain
			best_value = value
			best_total_l = total_l
			best_total_r = total_r
		}
	}

	return best_gain, best_value, best_total_l, best_total_r
}

func splitSamplesUint8(samples [][]uint8, c int, value uint8, part_l *[]int, part_r *[]int) {
	for j := 0; j < len(samples); j++ {
		if samples[j][c] <= value {
			*part_l = append(*part_l, j)
		} else {
			*part_r = append(*part_r, j)
		}
	}
}

func buildTreeUint8(samples [][]uint8, samples_labels []string, selected_feature_count int) *TreeNodeUint8 {
	//fmt.Println(len(samples))
	//find a best splitter
	column_count := len(samples[0])
	//split_count := int(math.Log(float64(column_count)))
	split_count := selected_feature_count
	columns_choosen := getRandomRange(column_count, split_count)

	best_gain := 0.0
	var best_part_l []int = make([]int, 0, len(samples))
	var best_part_r []int = make([]int, 0, len(samples))
	var best_total_l int = 0
	var best_total_r int = 0
	var best_value uint8
	var best_column int

	current_entropy_map := make(map[string]float64)
	for i := 0; i < len(samples_labels); i++ {
		current_entropy_map[samples_labels[i]] += 1
	}

	current_entropy := getEntropy(current_entropy_map, len(samples_labels))

	for _, c := range columns_choosen {
		gain, value, total_l, total_r := getBestGainUint8(samples, c, samples_labels, current_entropy)
		//fmt.Println("kkkkk",gain,part_l,part_r)
		if gain >= best_gain {
			best_gain = gain
			best_value = value
			best_column = c
			best_total_l = total_l
			best_total_r = total_r
		}
	}

	if best_gain > 0 && best_total_l > 0 && best_total_r > 0 {
		node := &TreeNodeUint8{}
		node.Value = best_value
		node.ColumnNo = best_column
		splitSamplesUint8(samples, best_column, best_value, &best_part_l, &best_part_r)
		node.Left = buildTreeUint8(getSamplesUint8(samples, best_part_l), getLabels(samples_labels, best_part_l), selected_feature_count)
		node.Right = buildTreeUint8(getSamplesUint8(samples, best_part_r), getLabels(samples_labels, best_part_r), selected_feature_count)
		return node
	}

	return genLeafNodeUint8(samples_labels)

}

func genLeafNodeUint8(labels []string) *TreeNodeUint8 {
	counter := make(map[string]int)
	for _, v := range labels {
		counter[v]++
	}

	node := &TreeNodeUint8{}
	node.Labels = counter
	//fmt.Println(node)
	return node
}

func predicateUint8(node *TreeNodeUint8, input []uint8) map[string]int {
	if node.Labels != nil { //leaf node
		return node.Labels
	}

	c := node.ColumnNo
	value := input[c]

	if value <= node.Value && node.Left != nil {
		return predicateUint8(node.Left, input)
	} else if node.Right != nil {
		return predicateUint8(node.Right, input)
	}

	return nil
}

func BuildTreeUint8(inputs [][]uint8, labels []string, samples_count, selected_feature_count int) *TreeUint8 {

	samples := make([][]uint8, samples_count)
	samples_labels := make([]string, samples_count)
	for i := 0; i < samples_count; i++ {
		j := int(rand.Float64() * float64(len(inputs)))
		samples[i] = inputs[j]
		samples_labels[i] = labels[j]
	}

	tree := &TreeUint8{}
	tree.Root = buildTreeUint8(samples, samples_labels, selected_feature_count)

	return tree
}

func PredicateTreeUint8(tree *TreeUint8, input []uint8) map[string]int {
	return predicateUint8(tree.Root, input)
}
